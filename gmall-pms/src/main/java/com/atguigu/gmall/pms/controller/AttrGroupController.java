...
public class AttrGroupController {
    ...
    @GetMapping("withattrs/cat/{catId}")//根据
    public Resp<List<AttrGroupVO>> queryGroupWithAttrsByCatId(@PathVariable("catId") Long catId){
        List<AttrGroupVO> attrGroupVOs = attrGroupService.queryGroupWithAttrsByCatId(catId);
        return Resp.ok(attrGroupVOs);
    }



    @ApiOperation("根据分组id查询分组及组下的规格参数,维护关系")
    @GetMapping("withattr/{gid}")//根据
    public Resp<AttrGroupVO> queryGroupWithById(@PathVariable("gid") Long gid){
    ...
    public Resp<PageVo> queryByCidPage(@PathVariable("catId") Long catId,QueryCondition queryCondition){
       ...
       PageVo pageVo = attrGroupService.queryByCatIdPage(catId,queryCondition);
