...
import com.atguigu.gmall.usm.vo.UserBoundVO;
...
public class OmsListener {
    ...
    public void closeOrder(String orderToken){
        // 修改订单的状态为已关闭
        if (this.orderService.closeOrder(orderToken)==1){//修改订单状态就是更新操作，orderService调用关闭订单方法，如果返回1说明订单状态已修改
            // 发送消息给库存，解锁库存
    ...
    public void paySuccess(String orderToken){
        //更新订单状态
        if (this.orderService.success(orderToken)==1){//跟新成功，数据库影响条数为1，说明更新成功
        //减库存               //更新成功后发送消息用于减库存
            this.amqpTemplate.convertAndSend("WMS-EXCHANGE", "stock.minus", orderToken);//监听器在gmall.wms.listener;
            //给用户加积分        //支付成功 发送消息增加用户积分
            ...
            this.amqpTemplate.convertAndSend("UMS-EXCHANGE","user.broud",userBoundVO);
        }
    }
